...
class Initial_characterModel extends Model
{
	public function selectAll()
	{
		$request = $this->pdo->prepare('SELECT * FROM Initial_character');
        $request->execute();
        ...
        {
            $character_list[] = new Initial_character($result);
        }
	...
	{
		$request = $this->pdo->prepare('SELECT * FROM Initial_character WHERE id = :id');
        $request->bindValue(':id', $id);
        $request->execute();

        if(($result = $request->fetch()) != false)
        {
            return new Initial_character($result);
        }
        else
        {
            return false;
	...
	{	
		$request = $this->pdo->prepare('INSERT INTO Initial_character
	...
	{
		$request = $this->pdo->prepare('UPDATE Initial_character
										SET health_max = :health_max,
											health     = :health,
											strength   = :strength,
											resistance = :resistance,
	...
	{
		$request = $this->pdo->prepare('DELETE FROM Initial_character WHERE id = :id');
        $request->bindValue(':id', $id);
        $request->execute();
	}
}