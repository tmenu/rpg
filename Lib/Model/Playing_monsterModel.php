...
use Lib\Entity\Playing_monster;

class Playing_monsterModel extends Model
{
	public function selectAll()
	{
		$request = $this->pdo->prepare('SELECT * FROM Playing_monster');
        $request->execute();
        ...
        {
            $character_list[] = new Playing_monster($result);
        }
	...
	{
		$request = $this->pdo->prepare('SELECT * FROM Playing_monster WHERE id = :id');
        $request->bindValue(':id', $id);
        $request->execute();

        if(($result = $request->fetch()) != false)
        {
            return new Playing_monster($result);
        }
        else
        {
            return false;
	...
	{	
		$request = $this->pdo->prepare('INSERT INTO Playing_monster
	...
	{
		$request = $this->pdo->prepare('UPDATE Playing_monster
										SET health_max = :health_max,
											health     = :health,
											strength   = :strength,
											resistance = :resistance,
	...
	{
		$request = $this->pdo->prepare('DELETE FROM Playing_monster WHERE id = :id');
        $request->bindValue(':id', $id);
        $request->execute();
	}
}